...
impl ManifestNode {
    ...
    pub fn build(container: &WalkerContainer, tree_id: usize, is_root: bool) -> String {
        ...
        let children: String = tree
            .children
            .iter()
            .map(|c| ManifestNode::build(&container, *c, false))
            .collect();
...
impl Manifest {
    pub fn build(container: &WalkerContainer) -> String {
        ...
        let root = ManifestNode::build(container, container.get_root_id().unwrap(), true);
        out.push_str(&root);
    ...
    pub fn update(container: &WalkerContainer, context: &CompileContext) -> io::Result<()> {
        let manifest = Manifest::build(container);
